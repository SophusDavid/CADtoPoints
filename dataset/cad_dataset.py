...
def get_dataloader(phase, config, shuffle=None):
    ...
    return dataloader
def get_GS_dataloader(phase, config, shuffle=None):
    is_shuffle = phase == 'train' if shuffle is None else shuffle

    dataset = GSCADDataset(phase, config)
    dataloader = DataLoader(dataset, batch_size=config.batch_size, shuffle=is_shuffle, num_workers=config.num_workers,
                            worker_init_fn=np.random.seed())
    return dataloader


class GSCADDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # images_path = os.path.join(self.img_data, data_id)
        # pose_path=os.path.join(self.img_data, data_id+"/transforms.json")
        ...
        #         from utils.camera_utils import PILtoTorch
        ...
        # return {"command": command, "args": args, "id": data_id, "images":images, "poses":poses,}
        return {"command": command, "args": args, "id": data_id,"points":points}
